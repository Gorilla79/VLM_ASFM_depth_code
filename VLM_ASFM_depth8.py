...
class ASFMNavigator:
    def __init__(self):
        ...
        self.yolo = YOLO('yolov8n.pt').to(self.device).half()
        self.processor = InstructBlipProcessor.from_pretrained("Salesforce/instructblip-vicuna-7b")
        self.vlm_model = InstructBlipForConditionalGeneration.from_pretrained(
            "Salesforce/instructblip-vicuna-7b",
            torch_dtype=torch.float16,  # ✅ FP16 적용
            device_map="auto"
        ).to(self.device)

        self.vlm_model.eval()  # ✅ Evaluation Mode
        self.vlm_model.half()  # ✅ FP16 적용 (Mixed Precision)
    ...
    def detect_humans(self, color_frame, depth_frame):
        color_image = np.asanyarray(color_frame.get_data()).astype(np.float16) / 255.0  # Normalize

        # ✅ YOLO 입력 처리 (Tensor 변환)
        input_tensor = torch.from_numpy(color_image).permute(2, 0, 1).unsqueeze(0).to(self.device).half()

        results = self.yolo(input_tensor)[0]
        humans = []
    ...
    def compute_adaptive_speed(self, humans):
        ...
        for human in humans:
            ...
            # ✅ 강제 STOP 조건: 정면 0.5m 이내 접근 시
            if dist < 0.5 and approaching and abs(human['position'][0]) < 0.3:  # 좌우 위치가 거의 정면
                return 0.0  # 강제 STOP
        ...
        try:
            while True:
                ...
                if not color_frame or not depth_frame:
                    continue
                color_image = np.asanyarray(color_frame.get_data()).astype(np.float16) / 255.0
                depth_image = np.asanyarray(depth_frame.get_data()).astype(np.float32) / 1000.0  # ✅ mm → meters
                ...
                if len(humans) >= 3:
                    ...
                    if len(humans) >= 3:
                        # VLM 사용
                        action, vlm_speed = self.generate_vlm_command(color_image, situation_text)
                        direction_map = {
                        ...
                        vlm_direction = direction_map.get(action, np.array([0, 0, 0]))
                
                        # ASFM 계산
                        force = self.compute_asfm_force(np.array([0, 0, 0]), humans)
                        force_mag = np.linalg.norm(force)
                        if force_mag < 0.1:  # 🔥 Force가 약하면 STOP
                            asfm_direction = np.array([0, 0, 0])  # STOP
                            asfm_speed = 0.0
                        else:
                            asfm_direction = force / (force_mag + 1e-6)
                            asfm_speed = self.compute_adaptive_speed(humans)
                
                        # VLM과 ASFM 결과 비교
                        direction_diff = np.linalg.norm(vlm_direction - asfm_direction)
                        if direction_diff < 0.5:
                            move_direction = vlm_direction
                            speed = vlm_speed
                        else:
                            move_direction = asfm_direction
                            speed = asfm_speed
                    else:
                        # ASFM만 사용
                        force = self.compute_asfm_force(np.array([0, 0, 0]), humans)
                        force_mag = np.linalg.norm(force)
                        if force_mag < 0.1:  # 🔥 STOP 조건
                            move_direction = np.array([0, 0, 0])  # STOP
                            speed = 0.0
                        else:
